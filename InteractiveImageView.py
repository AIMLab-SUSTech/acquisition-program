...
class InteractiveImageView(QGraphicsView):
    ...
    def update_image(self, image_data, show_mask=False):
        ...
        if getattr(self, 'v_line', None): 
            self.scene.removeItem(self.v_line); self.v_line = None
        if getattr(self, 'h_line', None): 
            self.scene.removeItem(self.h_line); self.h_line = None
        if getattr(self, 'circle', None): 
            self.scene.removeItem(self.circle); self.circle = None
        ...
        if show_mask:
            ...
            self.v_line.setZValue(10); self.h_line.setZValue(10); self.circle.setZValue(10)
